...
def test_register_user_success(api_client, mock_send_email):
    ...
    assert User.objects.filter(username="newuser123").exists(), (
        "Пользователь с username 'newuser123' не был создан в базе данных"
    )
    assert User.objects.filter(email="newuser@test.com").exists(), (
        "Пользователь с email 'newuser@test.com' не был создан в базе данных"
    )
...
# def test_register_user_calls_auth_service(mock_get_auth_service, api_client, mock_send_email):
#     # Given
#     url = reverse('register')
#     data = {
#         "email": "test@example.com",
...
#     # Мокаем AuthService
#     mock_service = Mock()
#     mock_service.register_user.return_value = type('UserDTO', (), {
#         'id': 1,
#         'email': 'test@example.com',
#         'username': 'testuser'
#     })()
