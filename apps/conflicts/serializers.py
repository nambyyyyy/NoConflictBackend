...
class ConflictDetailSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            conflict = ConflictModel.objects.create(**validated_data)
            ...
            for item_data in items_data_list:
                ...
                conflict_item = ConflictItemModel.objects.create(
                    conflict=conflict, item_type=item_data.get("item_type")
                )

                all_options_for_item = list(
                    OptionChoiceModel.objects.filter(id__in=predefined_ids)
                )

                for value in custom_values:
                    # Здесь смотрим, были ли уже ранее добавлены такие ответы
                    option, _ = OptionChoiceModel.objects.get_or_create(
                        value__iexact=value,
                ...
                creator_choice_obj = next(
                    (
                        opt
                        for opt in all_options_for_item
                        if opt.value == creator_choice_value
                    ),
                    None,
                )
                if creator_choice_obj:
                    conflict_item.creator_choice = creator_choice_obj  # type: ignore
